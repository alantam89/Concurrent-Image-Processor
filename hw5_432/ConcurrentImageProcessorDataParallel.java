 ...
 * @author alan
 *
 *ConcurrentImageProcessorDataParallel is a class that demonstrates concurrency using threads and semaphores.
 *Compared to the Task Parallel version, this class is data parallel. Given a number of processor threads, this class 
 *will issue a process filter command that will distribute the work of processing an image across all threads until all
 ...
 *processor threads - the read-queue is polled for an image. The image is divided into equal partitions for each thread to filter.
 *                    Once all threads are finished filtering, the image is put into the processed-queue.
